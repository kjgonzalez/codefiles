...
\begin{enumerate}\itemsep=-0.5em
    \item Sort scores and corresponding outcomes by descending confidence
    \item Identify unique index locations for approximate threshold indices
    \item Create "cumulative sum" TP-vector and FP vectors
    \item Remove extra indices after there are no more TP outcomes
    \item append theoretical (recall=0,precision=1) point to list
    \item OPTIONAL: Sort precision \& recall by 
\end{enumerate}
