...
KJG190807: XX THIS SECTION WILL EVENTUALLY BE REMOVED, AND ALL INFORMATION HEREIN WILL BE MOVED TO THE TOPIC1(PSMNET) AND TOPIC2(FPNET) SECTIONS
...
KJG190807: XX ADJUSTING PERFORMANCE METRICS HERE TO PROPERLY EXPLAIN THINGS, AND SEPARATING THE EXAMPLES FOR THE APPENDIX
...
Theoretically, precision and recall are generated at multiple points by varying a probability threshold and graphing each result. In practice, a precision-recall curve is generated by a more complex process that involves taking the cumulative sum of the detections in order of descending confidence level (1.0,0.99, ...). The steps are outlined below. These steps are taken from the popular Python module ``scikit-learn", provided by \cite{pedregosa_scikit-learn:_2011}. Although the below example is not exactly the same as what is performed in the official KITTI evaluation code, developed by David Stutz and Bo Li [cite xx], the general steps are similar.

The general steps for generating a precision-recall curve are as follow:
