...
\subsubsection{delme, lidar economics}
...
In order to make a more quantitative comparison between these two ranged sensors, an example will be taken from the KITTI setup. In collecting the necessary information to create the various datasets, \cite{geiger_are_2012} used a Velodyne HDL-64E lidar and two PointGrey Flea 2 color cameras. In 2012, when the paper was first published, an HDL-64E cost in the range of 75,000 USD. Although exact price information for the cameras, which have been discontinued, is difficult to find, a similar model (FL2-03S2C-C) seems to have been priced around 700 USD for a single camera; the assumption will be made that the camera system would have had a total cost of 1,400 USD. Although this clearly demonstrates that the cameras are cheaper, an attempt to normalize for pointcloud density / quality will be made next. See Table xx below for more information on both sensor setups. 
