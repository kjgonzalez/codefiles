'''
use scikit-learn library to create a k-nearest-neighbors classifier. train it on iris
...
skknn = KNeighborsClassifier(10)
skknn.fit(ds_train,labels_train)


ypred = skknn.predict(ds_test)
ytrue = labels_test[:]

# as an example, show probabilities when predicting
print(skknn.predict_proba(ds_test[:10]).round(3))
